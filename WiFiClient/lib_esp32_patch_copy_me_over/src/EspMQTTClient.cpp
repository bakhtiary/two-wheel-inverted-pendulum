...
bool EspMQTTClient::publishRaw(const String &topic, uint8_t * payload, unsigned int payload_length, bool retain)
{
  // Do not try to publish if MQTT is not connected.
  if(!isConnected())
  {
  ...
  bool success = _mqttClient.publish(topic.c_str(), payload, payload_length, retain);
  ...
  for (std::size_t i = 0 ; i < _topicSubscriptionList.size() ; i++)
  {
    if (mqttTopicMatch(_topicSubscriptionList[i].topic, String(topic)))
    {
      if(_topicSubscriptionList[i].callback != NULL)
        _topicSubscriptionList[i].callback(payload, length); // Call the callback
